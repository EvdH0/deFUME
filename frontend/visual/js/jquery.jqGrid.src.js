	...
	filterToolbar : function(p){
		...
		return this.each(function(){
           ...
           var  ess = function() {
               ...
               var ruleGroup = '{"groupOp":"AND","rules":[{"field":"h_desc","op":"bw","data":"hypothetical"}]}';
               $('[id^=list][id$=_t]').each(function () {
                   console.log(this)
                   //var p = $(this).getGridParam('postData');
                   //p.filters = ruleGroup;
                  $(this).setGridParam({ postData: p, search: true }).trigger("reloadGrid", [{ page: 2}]);
               });
               //alert('im fired')
               console.log('Jaja fired')
            }
var test = function(){
    alert('ben ik');
